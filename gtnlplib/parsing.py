...
class ParserState:
    ...
    def done_parsing(self):
        ...
        """
        return True if self.input_buffer_peek_n(1)[0].headword == END_OF_INPUT_TOK and self.stack_len() == 1 else False
        # END STUDENT
    ...
    def _create_arc(self, head, modifier):
        ...
        """
        #STUDENT
        self.input_buffer.insert(0, StackEntry(head.headword, head.headword_pos, self.combiner(head.embedding, modifier.embedding)))
    ...
    def _validate_action(self, action_to_do):
        ...
        if action_to_do == Actions.SHIFT:
            if self.input_buffer_len() <= 2:
                return Actions.ARC_R
            else:
                return Actions.SHIFT

        if action_to_do == Actions.ARC_L:
            if self.stack_peek_n(1)[0].headword == ROOT_TOK:
                if self.input_buffer_len() == 2:
                    return Actions.ARC_R
                else:
                    return Actions.SHIFT
            else:
                return Actions.ARC_L
...
class TransitionParser(nn.Module):
    ...
    def forward(self, sentence, actions=None):
        ...
        while not parser_state.done_parsing():
            outputs.append(self.action_chooser(self.feature_extractor.get_features(parser_state)))
            if have_gold_actions:
                actions_done.append(action_queue.pop())
                if actions_done[-1] == Actions.SHIFT:
                    
                    parser_state.shift()
                elif actions_done[-1] == Actions.ARC_L:
                    
                    dep_graph.add(parser_state.arc_left())
                elif actions_done[-1] == Actions.ARC_R:
                    
                    dep_graph.add(parser_state.arc_right())
            else:
                action = utils.argmax(outputs[-1])
                action_to_do = parser_state._validate_action(action)
                if action_to_do == Actions.SHIFT:
                    parser_state.shift()
                    actions_done.append(Actions.SHIFT)
                elif action_to_do == Actions.ARC_R:
                    dep_graph.add(parser_state.arc_right())
                    actions_done.append(Actions.ARC_R)
                elif action_to_do == Actions.ARC_L:
                    dep_graph.add(parser_state.arc_left())
                    actions_done.append(Actions.ARC_L)
