...
class VanillaWordEmbedding(nn.Module):
    ...
    def forward(self, sentence):
        ...
        var = ag.Variable(torch.LongTensor([self.word_to_ix[t] for t in sentence]))
        return [self.word_embeddings(j) for j in var]
...
class BiLSTMWordEmbedding(nn.Module):
    ...
    def forward(self, sentence):
        ...
        # STUDENT
        i = ag.Variable( torch.LongTensor([self.word_to_ix[t] for t in sentence]))
        inp = self.word_embeddings(i).resize(len(sentence), 1, self.word_embedding_dim)
...
class SuffixAndWordEmbedding(nn.Module):
    ...
    def __init__(self, word_to_ix, suff_to_ix, embedding_dim):
        ...
        self.suffix_embeddings = nn.Embedding(len(self.suff_to_ix), self.embedding_dim//2)
        # END STUDENT
    ...
    def forward(self, sentence):
        ...
        embeds = [] # store each Variable in here
        # STUDENT
        word_var = ag.Variable(torch.LongTensor([self.word_to_ix[t] for t in sentence]))
        word_embed = [self.word_embeddings(j) for j in word_var]
        ...
        suff_var = ag.Variable(torch.LongTensor([self.suff_to_ix[t] for t in suff]))
        suff_embed = [self.suffix_embeddings(j) for j in suff_var]

        for i in range(len(word_embed)):
            embeds.append(torch.cat((word_embed[i], suff_embed[i]), 1))
...
class FFCombiner(nn.Module):
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        """
        # STUDENT
        return self.second_layer(F.tanh(self.first_layer(torch.cat([head_embed, modifier_embed], 1).view(1, -1))))
        # END STUDENT
...
class LSTMCombiner(nn.Module):
    ...
    def __init__(self, embedding_dim, num_layers, dropout):
        ...
        # STUDENT
        self.lstm = nn.LSTM(input_size=self.embedding_dim * 2, hidden_size=embedding_dim, num_layers=num_layers, dropout=dropout)
        # END STUDENT
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        """
        # STUDENT
        out, self.hidden = self.lstm(torch.cat([head_embed, modifier_embed], 1).view(1, -1).resize(self.num_layers, 1, self.embedding_dim * 2), self.hidden)
...
class FFActionChooser(nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        # STUDENT
        return F.log_softmax(self.second_layer(F.relu(self.first_layer(torch.cat(inputs, 1).view(1, -1)))), dim=1)
        # END STUDENT
...
class LSTMActionChooser(nn.Module):
    ...
    def __init__(self, input_dim, num_layers, dropout):
        ...
        self.lstm = nn.LSTM(input_size=self.input_dim, hidden_size=input_dim, num_layers=num_layers, dropout=dropout)
        self.linear = nn.Linear(input_dim, 3)
        self.hidden = self.init_hidden()
    
    def forward(self, inputs):  # size of inputs is input_dim
        """
        combine all the features into one big row vector, then compute log probabilities
        ...
        """
        # STUDENT
        out, self.hidden = self.lstm((torch.cat(inputs, 1).view(1, -1)), self.hidden)
        return F.log_softmax(self.linear(F.relu(out.view(1, self.input_dim))), dim=1)
        # END STUDENT
